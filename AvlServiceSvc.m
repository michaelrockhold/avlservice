#import "AvlServiceSvc.h"
#import <libxml/xmlstring.h>
@implementation AvlServiceSvc_AvlReport
- (id)init
{
	if((self = [super init])) {
		routeID = 0;
...
+ (AvlServiceSvc_AvlReport *)deserializeNode:(xmlNodePtr)cur
{
	AvlServiceSvc_AvlReport *newObject = [[AvlServiceSvc_AvlReport new] autorelease];
...
@end
@implementation AvlServiceSvc_ArrayOf_xsd_string
- (id)init
{
	if((self = [super init])) {
	}
...
- (NSString *)serializedFormUsingElementName:(NSString *)elName
{
	...
	[serializedForm appendFormat:@"<%@ xsi:type=\"AvlServiceSvc:ArrayOf_xsd_string\"", elName];
	[serializedForm appendString:[self serializedAttributeString]];
	[serializedForm appendFormat:@">"];
...
+ (AvlServiceSvc_ArrayOf_xsd_string *)deserializeNode:(xmlNodePtr)cur
{
	AvlServiceSvc_ArrayOf_xsd_string *newObject = [[AvlServiceSvc_ArrayOf_xsd_string new] autorelease];
...
@end
@implementation AvlServiceSvc_ArrayOfAvlReport
- (id)init
{
	if((self = [super init])) {
	}
...
- (NSString *)serializedFormUsingElementName:(NSString *)elName
{
	...
	[serializedForm appendFormat:@"<%@ xsi:type=\"AvlServiceSvc:ArrayOfAvlReport\"", elName];
	[serializedForm appendString:[self serializedAttributeString]];
	[serializedForm appendFormat:@">"];
...
+ (AvlServiceSvc_ArrayOfAvlReport *)deserializeNode:(xmlNodePtr)cur
{
	AvlServiceSvc_ArrayOfAvlReport *newObject = [[AvlServiceSvc_ArrayOfAvlReport new] autorelease];
...
@end
@implementation AvlServiceSvc
+ (void)initialize
{
	[[USGlobals sharedInstance].wsdlStandardNamespaces setObject:@"" forKey:@"http://www.w3.org/2001/XMLSchema"];
	[[USGlobals sharedInstance].wsdlStandardNamespaces setObject:@"AvlServiceSvc" forKey:@"http://avl.transit.ws.its.washington.edu"];
				...
				for(bodyNode=cur->children ; bodyNode != NULL ; bodyNode = bodyNode->next) {
					if(cur->type == XML_ELEMENT_NODE) {
						if(xmlStrEqual(bodyNode->name, (const xmlChar *) "ArrayOf_xsd_string")) {
							AvlServiceSvc_ArrayOf_xsd_string *bodyObject = [AvlServiceSvc_ArrayOf_xsd_string deserializeNode:bodyNode];
							NSAssert1(bodyObject != nil, @"Errors while parsing body %s", bodyNode->name);
							[responseBodyParts addObject:bodyObject];
						}
					}
				...
				for(bodyNode=cur->children ; bodyNode != NULL ; bodyNode = bodyNode->next) {
					if(cur->type == XML_ELEMENT_NODE) {
						if(xmlStrEqual(bodyNode->name, (const xmlChar *) "ArrayOfAvlReport")) {
							AvlServiceSvc_ArrayOfAvlReport *bodyObject = [AvlServiceSvc_ArrayOfAvlReport deserializeNode:bodyNode];
							NSAssert1(bodyObject != nil, @"Errors while parsing body %s", bodyNode->name);
							[responseBodyParts addObject:bodyObject];
						}
					}
...
- (NSString *)serializedFormUsingHeaderElements:(NSDictionary *)headerElements bodyElements:(NSDictionary *)bodyElements
{
	...
	[serializedForm appendFormat:@"  xmlns:AvlServiceSvc=\"http://avl.transit.ws.its.washington.edu\"\n"];
